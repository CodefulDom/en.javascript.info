...
![](../../../.gitbook/assets/try-catch-flow.svg)
...
* An example with an error: shows `(1)` and `(3)`:
  ...
  _!_ lalala; // error, variable is not defined! _/!_
...
\`\`\`js run try { {{{{{{{{{{{{ } catch\(e\) { alert\("The engine can't understand this code, it's invalid"\); }
...
```````warn header="````try..catch`works synchronously" If an exception happens in "scheduled" code, like in`setTimeout`, then`try..catch\` won't catch it:

\`\`\`js run try { setTimeout\(function\(\) { noSuchVariable; // script will die here }, 1000\); } catch \(e\) { alert\( "won't work" \); }
...
\`\`\`html run untrusted refresh height=1
 ...
 _!_ window.onerror = function\(message, url, line, col, error\) { alert\(`${message}\n At ${line}:${col} of ${url}`\); }; _/!_
