...
`````smart header="Always prefer```wss://`" The`wss://\` protocol not only encrypted, but also more reliable.
...
For demo purposes, there's a small server [server.js](https://github.com/CodefulDom/en.javascript.info/tree/a035351fcfceb747760a1d9bd2c652f624999a4a/5-network/11-websocket/demo/server.js) written in Node.js, for the example above, running. It responds with "Hello from server, John", then waits 5 seconds and closes the connection.
