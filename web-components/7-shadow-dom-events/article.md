...
&lt;/user-card&gt;
 ...
 customElements.define\('user-card', class extends HTMLElement { connectedCallback\(\) { this.attachShadow\({mode: 'open'}\); this.shadowRoot.innerHTML = `<p> <button>Click me</button> </p>`; this.shadowRoot.firstElementChild.onclick = e =&gt; alert\("Inner target: " + e.target.tagName\); } }\);

document.onclick = e =&gt; alert\("Outer target: " + e.target.tagName\); &lt;/script&gt;
