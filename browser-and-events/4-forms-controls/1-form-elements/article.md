...
_!_ alert\(ageElems\[0\]\); // \[object HTMLInputElement\] _/!_ &lt;/script&gt;
...
![](../../../.gitbook/assets/form-navigation.svg)

For instance:

\`\`\`html run height=40

 _!_ // form -&gt; element let login = form.login;

// element -&gt; form alert\(login.form\); // HTMLFormElement _/!_ &lt;/script&gt;
...
`````warn header="Use```textarea.value`, not`textarea.innerHTML`" Please note that even though`...`holds its value as nested HTML, we should never use`textarea.innerHTML\` to access it.
...
BluesRockClassic
 ...
 // get all selected values from multi-select let selected = Array.from\(select.options\) .filter\(option =&gt; option.selected\) .map\(option =&gt; option.value\);
...
* Specification: [https://html.spec.whatwg.org/multipage/forms.html](https://html.spec.whatwg.org/multipage/forms.html).
