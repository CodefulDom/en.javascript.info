...
Alt+Shift+Click on me!
 ...
 button.onclick = function\(event\) { &lt;em&gt;!&lt;/em&gt; if \(event.altKey &amp;&amp; event.shiftKey\) { &lt;em&gt;/&lt;/em&gt;&lt;em&gt;!&lt;/em&gt; alert\(&apos;Hooray!&apos;\); } }; 
...
\`\`\`warn header="There are also mobile devices" Keyboard combinations are good as an addition to the workflow. So that if the visitor has a keyboard -- it works. And if their device doesn't have it -- then there should be another way to do the same.
