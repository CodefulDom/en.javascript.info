...
alert\( slow\(1\) \); // slow\(1\) is cached alert\( "Again: " + slow\(1\) \); // the same

alert\( slow\(2\) \); // slow\(2\) is cached alert\( "Again: " + slow\(2\) \); // the same as the previous line
...
function cachingDecorator\(func\) { let cache = new Map\(\); return function\(x\) { if \(cache.has\(x\)\) { return cache.get\(x\); } _!_ let result = func.call\(this, x\); // "this" is passed correctly now _/!_ cache.set\(x, result\); return result; }; }
